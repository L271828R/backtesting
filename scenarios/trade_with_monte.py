...
def simulate_trading(session_results):
    """
    Simulate the trading strategy:
      - Trade $1 per session normally.
      - If the 16:00 trade is profitable, you gain 10% of the trade amount.
      - If the 16:00 trade is unprofitable, you lose 100% of the trade amount.
      - If you lose on a day (when trading $1), then the next session you trade $5 (scaled trade)
        for that day only. After that, revert to trading $1.
        
    Returns a list of dictionaries that include the original session result plus:
      - 'trade_amount': the dollar amount risked that session.
      - 'simulated_profit': profit from the session based on the trade amount.
    """
    # Sort session_results by date (session assumed in ISO format "YYYY-MM-DD")
    sorted_results = sorted(
        session_results,
        key=lambda x: datetime.strptime(x["session"], "%Y-%m-%d")
    )
    ...
    simulation = []
    # Flag to indicate if next session is scaled (5x) because previous normal trade lost.
    scaled_next = False
    
    for res in sorted_results:
        # Determine trade amount: if scaled_next is True, use $5, otherwise $1.
        if scaled_next:
            trade_amount = 5.0
            # Reset the flag after using the scaled trade for one day.
            scaled_next = False
        else:
            trade_amount = 1.0
        
        # Outcome is based on the 16:00 trade.
        if res["profitable_16"]:
            # Win: gain 10% of trade amount.
            profit = trade_amount * 0.10
        else:
            # Loss: lose 100% of trade amount.
            profit = -trade_amount
            # If this loss occurred on a normal trade ($1), then flag next trade to be scaled.
            if trade_amount == 1.0:
                scaled_next = True
        
        # Add simulation info to the result.
        sim_res = res.copy()
        sim_res["trade_amount"] = trade_amount
        sim_res["simulated_profit"] = profit
        simulation.append(sim_res)
...
def main():
    ...
    # Simulate the trading strategy.
    simulation_results = simulate_trading(session_results)
    sim_df = pd.DataFrame(simulation_results)
    simulated_csv_file = "session_results_simulated.csv"
    sim_df.to_csv(simulated_csv_file, index=False)
    print(f"Simulated session results saved to '{simulated_csv_file}'.")

    # Compute simulation summary statistics.
    total_simulated_profit = sim_df['simulated_profit'].sum()
    average_simulated_profit = sim_df['simulated_profit'].mean() if len(sim_df) > 0 else 0
    win_rate = (sim_df[sim_df["simulated_profit"] > 0].shape[0] / len(sim_df))*100 if len(sim_df) > 0 else 0

    summary_lines.extend([
        "",
        "Simulated Trading Summary (Strategy: $1 per day; 5x trade next day after loss):",
        f"  Total simulated profit: ${total_simulated_profit:.2f}",
        f"  Average simulated profit per trade: ${average_simulated_profit:.2f}",
        f"  Win rate (by simulated profit): {win_rate:.2f}%",
    ])
