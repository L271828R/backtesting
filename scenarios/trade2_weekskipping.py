...
def analyze_session(session, df):
    ...
    This function looks for a 12:30 row to determine the trade rule:
      - If target_label is 'high': short at the vwap_upper price.
      - If target_label is 'low': buy at the vwap_lower price.
    ...
    if target_label.lower() == "high":
        entry_price = entry_row['vwap_upper']
    elif target_label.lower() == "low":
        entry_price = entry_row['vwap_lower']
    else:
...
def simulate_skip_rule(session_results):
    """
    Simulate the rule: if a trade loses (16:00 trade not profitable), then skip trading for 1 week.
    Sessions are assumed to have a date string ("YYYY-MM-DD").
    
    Adds the following keys to each session result:
      - 'traded': True if trade was taken; False if skipped.
      - 'profit_16_simulated': profit for the session (0 if skipped).
      - 'skip_reason': explanation if the session was skipped.
    """
    # Sort by session date.
    sorted_results = sorted(session_results, key=lambda x: datetime.strptime(x["session"], "%Y-%m-%d"))
    simulated = []
    next_allowed_date = None
    for res in sorted_results:
        session_date = datetime.strptime(res["session"], "%Y-%m-%d")
        if next_allowed_date is not None and session_date < next_allowed_date:
            # Skip this trade due to recent loss.
            res["traded"] = False
            res["profit_16_simulated"] = 0.0
            res["skip_reason"] = "Skipped due to previous loss (within 1 week)"
        else:
            res["traded"] = True
            res["profit_16_simulated"] = res["profit_16"]
            res["skip_reason"] = ""
            # If this trade loses, set the next allowed trade date to 7 days after this session.
            if res["profit_16"] <= 0:
                next_allowed_date = session_date + pd.Timedelta(days=7)
            else:
                next_allowed_date = None
...
def main():
    ...
    # Compute winning streaks (based on 16:00 profit).
    longest_streak, average_streak, shortest_streak = compute_winning_streaks(session_results)

    # Now simulate the skip rule: if a session loses, skip trading for the next 1 week.
    simulated_results = simulate_skip_rule(session_results)
    simulated_df = pd.DataFrame(simulated_results)
    simulated_csv_file = "session_results_simulated.csv"
    simulated_df.to_csv(simulated_csv_file, index=False)
    print(f"Simulated session results (with skip rule) saved to '{simulated_csv_file}'.")

    # Summary statistics for simulated trades (only for sessions where a trade was taken).
    traded_df = simulated_df[simulated_df["traded"] == True]
    total_traded = len(traded_df)
    avg_profit_simulated = traded_df['profit_16_simulated'].mean() if total_traded > 0 else None
    positive_trades = traded_df[traded_df["profit_16_simulated"] > 0].shape[0] if total_traded > 0 else 0
    ...
    summary_lines = [
        ...
        "",
        "Simulated Trading (Skip Rule) Summary:",
        f"  Total trades taken: {total_traded}",
        f"  Average profit at 16:00 for traded sessions: {avg_profit_simulated:.2f}" if avg_profit_simulated is not None else "  Average profit at 16:00 for traded sessions: N/A",
        f"  Profitable trades: {positive_trades} ({(positive_trades/total_traded*100):.2f}%)" if total_traded > 0 else "  Profitable trades: N/A",
    ]
    summary_text = "\n".join(summary_lines)
